...
import { Patient, Specialization, Case } from "../../Services/Paths";
...
export const CaseProvider = ({ children }) => {
  ...
  const initiliazedPatients = useCallback(async () => {
    try {
      const response = await GetRequest({ url: `${Patient}s` });
  ...
  const initiliazedSpecializations = useCallback(async () => {
    try {
      const path =
        user !== null ? `${Specialization}s/u` : `${Specialization}s`;
      const response = await GetRequest({ url: path });
  ...
  }, [status, user, initiliazedPatients, initiliazedSpecializations]);
